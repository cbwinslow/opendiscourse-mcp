#!/usr/bin/env python3
"""
Test database connectivity and configuration (simplified)
"""
...
def test_database_connectivity():
    ...
    try:
        ...
        print("✅ Job context cleared successfully")
        
        # Test 11: Alert System
        print("\n🚨 Test 11: Alert System")
        cursor.execute("SELECT * FROM get_ingestion_alerts() LIMIT 5")
        alerts = cursor.fetchall()
        print(f"✅ Alert system: {len(alerts)} sample alerts")
        
        # Test 12: Ingestion Jobs Table
        print("\n📋 Test 12: Ingestion Jobs Table")
        cursor.execute("SELECT COUNT(*) FROM ingestion_jobs")
        job_count = cursor.fetchone()[0]
        print(f"✅ Ingestion jobs table: {job_count} records")
        
        # Test 13: Performance Log Table
        print("\n📊 Test 13: Performance Log Table")
        cursor.execute("SELECT COUNT(*) FROM ingestion_performance_log")
        perf_count = cursor.fetchone()[0]
        print(f"✅ Performance log table: {perf_count} records")
        ...
        print(f"✅ Database Size: {db_size}")
        print(f"✅ Tables: {table_count}, Indexes: {index_count}")
